...
const Overlay = ({ id, name, address, onClick }: OverlayProps) => {
  return (
    <div className="relative w-52 h-34 rounded-md bg-white p-2 flex justify-center items-end border border-Green">
      ...
      <div className="flex space-between w-48 gap-1 mt-7 text-ellipsis z-10">
        <Image src={noImg} width={100} alt="캠핑장 미리보기" quality={20} />
        <div className="w-20 h-10 text-description text-Gray overflow-hidden">
          <div className="line-clamp-3">{address}</div>
        </div>
      </div>
    </div>
  );
