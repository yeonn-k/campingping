...
> 본 문서는 캠핑핑 프로젝트의 **리팩토링 내역**을 기록한 문서입니다.
> 프로젝트 기간 내 기본 구현 기능은 `DETAILS.md`,
> 프로젝트 종료 후 추가 기능 구현은 `FEATURES.md`에 정리되어 있습니다.

## 🫧 리팩토링 목적 및 기대 효과

- 코드 유지보수성과 확장성을 고려한 구조 개선
- 사용자 경험(UX) 향상을 위한 성능 최적화 및 흐름 단순화
- 데이터 흐름의 일관성 확보 및 비동기 처리 안정화
...
- **_React 쿼리 상태 감지 기능을 활용하여 새로고침 및 뒤로 가기, URL 공유 등의 상황에도 검색 필터 유지_**

- ✨ 검색 조건 초기화 없이 URL 공유 가능, UX 개선
...
- **_iOS에서 PWA `beforeinstallprompt`지원하지 않아 PWA 설치에 문제 발생했던 점을 안내 모달로 개선_**

- ✨ iOS 환경에서도 설치 가이드 제공, UX 불편 최소화

- 변경 전

  - `iOS`에서는 `beforeinstallprompt` 지원되지 않아 PWA 설치 불가 및 사용자에 안내되지 않음

- 변경 후

  - `beforeinstallprompt`가 지원되지 않는 경우 '홈 화면 추가' 안내 모달로 UX 개선

## 📍 캠핑장 상세페이지

- ✨ 데이터 누락에 대한 조건부 렌더링 처리, 정보 표시 향상
...
- 변경 전
  ...
  - 날씨 컴포넌트는 사용자가 보고 있는 캠핑장과 관계 없이 사용자의 현재 위치의 날씨를 보여줌
  - 없는 페이지의 경우 계속해서 로딩중이라는 문구만 떠 있는 상태

- 변경 후

  - 값이 있는 지 체크하여 조건부 렌더링 처리
  - 가능한 많은 정보를 사용자에게 제공할 수 있도록 함
...
- ✨ 서버 사이드 메타데이터 정상 활용 가능, SEO 및 초기 데이터 최적화
...
- ✨ 불필요한 props 제거 및 가벼운 대체 컴포넌트로 최적화
...
- ✨ 채팅 기능 관리 및 컴포넌트 코드 정리

- 변경 전

  - 채팅 관련 send, receive, read 등의 로직이 컴포넌트마다 중복 및 분산

- 변경 후

  - 채팅 기능을 `useChat` 커스텀 훅으로 분리
  - 컴포넌트에서 `useChat` 훅 호출하여 사용
  - 재사용성과 코드 가독성 향상

  ### 🔧 변경된 파일

  - src/hooks/chat/useChat.tsx

## 📍 지도 페이지

### 🗺️ 무한 스크롤 제거

- ✨ 렌더링 최적화, 사용자의 초기 데이터 접근성 향상
...
### 🗺️ 데이터 패칭 방식 개선( TanStack Query 기반 커스텀 훅 적용 )

- ✨ 데이터 캐싱 및 리페치 최적화, API 호출 성능 개선

- 변경 전

  - useEffect와 axios 요청을 직접 컴포넌트 내에서 관리

- 변경 후

  - `@tanstack/react-query` 기반으로 리팩토링하여 데이터 캐싱, 자동 리페치, 상태 관리 최적화
  - `useCampingsByLocation`, `useCampingsByDoNm` 커스텀 훅 분리

  ### 🔧 변경된 파일

  - src/hooks/queries/useCampingsByDoNm.ts
  - src/hooks/queries/useCampingsByLocation.ts
